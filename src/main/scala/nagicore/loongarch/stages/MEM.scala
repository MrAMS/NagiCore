...
class MEM extends Module with Config{
    ...
    // pipeline registers
    val preg = RegEnable(io.ex2mem.bits, !io.mem2wb.stall && !dmem.io.data.stall)

    io.ex2mem.stall := io.mem2wb.stall
    io.mem2wb.bits.valid := preg.valid && dmem.io.data.valid
    ...
    dmem.io.clk := clock
    dmem.io.rst := reset
    dmem.io.data.addr := addr
    dmem.io.data.wdata := Flags.onehotMux(preg.st_type, Seq(
    ...
    dmem.io.data.req := (preg.ld_type =/= Flags.bp(CtrlFlags.ldType.x) || preg.st_type =/= Flags.bp(CtrlFlags.stType.x)) && preg.valid
    dmem.io.data.wmask := Flags.onehotMux(preg.st_type, Seq(
        CtrlFlags.stType.x  -> 0.U,
        CtrlFlags.stType.b  -> ("b1".U<<addr(1, 0)),
        CtrlFlags.stType.h  -> ("b11".U<<(addr(1)##0.U(1.W))),
        CtrlFlags.stType.w  -> "b1111".U,
    ))
    dmem.io.data.size := Flags.onehotMux(preg.st_type, Seq(
        CtrlFlags.stType.x  -> 0.U,
    ...
    io.mem2wb.bits.rdata := dmem.io.data.rdata
