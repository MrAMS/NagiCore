...
object DecoderMap{
    ...
    def make_instr_list(
        ...
        instr_type: String  = CtrlFlags.InstrType.alu,
        ill_instr: String   = "0")
        = Map(
            "regB_sel"->regB_sel,
            "regC_sel"->regC_sel,
    ...
    val instr_list_map : Seq[(BitPat, Map[String, String])]   = Seq(
        ...
        MULW        -> make_instr_list(alu_op=ALU_OP.MUL, instr_type=CtrlFlags.InstrType.alu2),
        ...
        BEQ         -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.X, br_type=BR_TYPE.EQ,    imm_type=ImmType.offs16, instr_type=CtrlFlags.InstrType.br),
        BNE         -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.X, br_type=BR_TYPE.NE,    imm_type=ImmType.offs16, instr_type=CtrlFlags.InstrType.br),
        BLT         -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.X, br_type=BR_TYPE.LT,    imm_type=ImmType.offs16, instr_type=CtrlFlags.InstrType.br),
        BGE         -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.X, br_type=BR_TYPE.GE,    imm_type=ImmType.offs16, instr_type=CtrlFlags.InstrType.br),
        BLTU        -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.X, br_type=BR_TYPE.LTU,   imm_type=ImmType.offs16, instr_type=CtrlFlags.InstrType.br),
        BGEU        -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.X, br_type=BR_TYPE.GEU,   imm_type=ImmType.offs16, instr_type=CtrlFlags.InstrType.br),
        // PC = PC + SignExtend({offs26, 2'b0}, 32)
        B           -> make_instr_list(regC_sel=regType.x, alu_op=ALU_OP.X, br_type=BR_TYPE.ALWAYS, imm_type=ImmType.offs26, instr_type=CtrlFlags.InstrType.br),
        // GR[1] = PC + 4; PC = PC + SignExtend({offs26, 2'b0}, 32)
        BL          -> make_instr_list(regC_sel=regType.num1, alu_op=ALU_OP.ADD, aluA_sel=CtrlFlags.aluASel.pc, aluB_sel = CtrlFlags.aluBSel.num4, br_type=BR_TYPE.ALWAYS, imm_type=ImmType.offs26, instr_type=CtrlFlags.InstrType.br),
        // GR[rd] = PC + 4; PC = GR[rj] + SignExtend({offs16, 2'b0}, 32)
        JIRL        -> make_instr_list(alu_op=ALU_OP.ADD, aluA_sel=CtrlFlags.aluASel.pc, aluB_sel = CtrlFlags.aluBSel.num4, br_type=BR_TYPE.ALWAYS, imm_type=ImmType.offs16, brpcAdd_sel = CtrlFlags.brpcAddSel.ra_val, instr_type=CtrlFlags.InstrType.br),


        LDB         -> make_instr_list(alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, ld_type=CtrlFlags.ldType.b, instr_type=CtrlFlags.InstrType.ls),
        LDBU        -> make_instr_list(alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, ld_type=CtrlFlags.ldType.bu, instr_type=CtrlFlags.InstrType.ls),
        LDH         -> make_instr_list(alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, ld_type=CtrlFlags.ldType.h, instr_type=CtrlFlags.InstrType.ls),
        LDHU        -> make_instr_list(alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, ld_type=CtrlFlags.ldType.hu, instr_type=CtrlFlags.InstrType.ls),
        LDW         -> make_instr_list(alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, ld_type=CtrlFlags.ldType.w, instr_type=CtrlFlags.InstrType.ls),

        STB         -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, st_type=CtrlFlags.stType.b, instr_type=CtrlFlags.InstrType.ls),
        STH         -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, st_type=CtrlFlags.stType.h, instr_type=CtrlFlags.InstrType.ls),
        STW         -> make_instr_list(regB_sel=regType.rd, regC_sel=regType.x, alu_op=ALU_OP.ADD, aluB_sel=CtrlFlags.aluBSel.imm, st_type=CtrlFlags.stType.w, instr_type=CtrlFlags.InstrType.ls),
...
class Decoder(XLEN: Int, GPR_LEN: Int) extends Module{
    val io = IO(new Bundle {
        ...
        val instr_type  = Output(CtrlFlags.InstrType())
    ...
    io.instr_type := decode_signal("instr_type")
