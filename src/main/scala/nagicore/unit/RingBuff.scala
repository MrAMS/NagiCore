...
class RingBuffIO[T <: Bundle](dataT: ()=> T, len: Int) extends Bundle{
    val full    = Output(Bool())
    val empty   = Output(Bool())

    val push    = Input(Bool())
    val whead   = Input(dataT())
    val pop     = Input(Bool())
    val rtail   = Output(dataT())
    val clear   = Input(Bool())

    val buffers = Output(Vec(len, dataT()))
    val valids = Output(Vec(len, Bool()))
}

class RingBuff[T <: Bundle](dataT: ()=> T, len: Int, id: Int=0) extends Module{
    require((len&(len-1))==0)
    val io = IO(new RingBuffIO(dataT, len))
    ...
    io.empty := empty
    io.full := full
    io.rtail := buff(buff_head)
    io.buffers := buff
    io.valids := buff_valid

    when(io.push && !full){
        buff_tail := buff_tail + 1.U
        buff(buff_tail) := io.whead
